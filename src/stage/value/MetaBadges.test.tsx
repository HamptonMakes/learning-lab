...
describe('MetaBadges', () => {
  it('renders every meta key as an addressable badge in a fixed order', () => {
    ...
    expect(node(container, 'alice.v@ts').dataset.value).toBe('t3')
    ...
    expect(node(container, 'alice.v@addTs').textContent).toBe('add t1')
    expect(node(container, 'alice.v@removeTs').textContent).toBe('remove t2')
