  ...
  scenes: [
    scene(
      ...
      [
        ...
        step(
          's02',
          'Say what a CRDT is: a data type whose merge rule is fixed up front, so copies change alone and still end up equal ([I.5](/crdts/the-problem/meet-crdts)).',
          insert('board.done', item('k1', 'What a CRDT is')),
          check('board.done[k1]', { sticky: true, id: 'c1' }),
        ),
        step(
          's03',
          'State the three merge laws: order does not matter, grouping does not matter, merging twice is harmless ([II.1](/crdts/state-based/the-shape-of-a-state-crdt)).',
          insert('board.done', item('k2', 'The three merge laws')),
          check('board.done[k2]', { sticky: true, id: 'c2' }),
        ),
        step(
          's04',
          'Tell the two styles apart: send the whole state and merge, or send what you did, once and in causal order ([III.1](/crdts/operation-based/ops-instead-of-state)).',
          insert('board.done', item('k3', 'State-based vs ops')),
          check('board.done[k3]', { sticky: true, id: 'c3' }),
        ),
        step(
          's05',
          'Use registers and counters: a register replaces and needs a stamp, a counter adds up with one tally per node ([II.2](/crdts/state-based/lww-register)).',
          insert('board.done', item('k4', 'Registers and counters')),
          check('board.done[k4]', { sticky: true, id: 'c4' }),
        ),
        step(
          's06',
          'Pick a set: G only grows, 2P never re-adds, LWW-element decides by time, OR-Set tags every add ([II.9](/crdts/state-based/or-set)).',
          insert('board.done', item('k5', 'Sets (G, 2P, LWW, OR)')),
          check('board.done[k5]', { sticky: true, id: 'c5' }),
        ),
        step(
          's07',
          'Keep an order: a sequence inserts after an id and keeps tombstones, so every copy reads the same list ([III.5](/crdts/operation-based/sequences-rga)).',
          insert('board.done', item('k6', 'Sequences (RGA)')),
          check('board.done[k6]', { sticky: true, id: 'c6' }),
        ),
        step(
          's08',
          'Read a clock: wall clocks lie, Lamport gives an order, vector clocks tell before, after or concurrent ([IV.3](/crdts/vector-clocks/vector-clocks)).',
    ...
    scene(
      ...
      [
        ...
        step(
          's06',
          'An **invariant** like that needs coordination: a lock, a transaction, or one owner of the value ([I.2](/crdts/the-problem/locks-the-classic-answer)).',
