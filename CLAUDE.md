...
| Design language    | "Workbench" (a beige computer on a desk, 1991). Two worlds: **hardware** — putty-beige page and sidebar, ivory panels, keycap buttons (orange power key = play), a bezel around the stage, LED readouts in Doto — and **screen** — the stage is a cool grey monitor (faint dither) where every actor is a white window with a striped title bar and a hard shadow; boards are sticky notes. One accent (CRT cyan), a small semantic palette for actors (Alice / Bob / Carol / Server each own a hue). Archivo for UI, JetBrains Mono for data values, Doto only for LED digits. Readable first, then fun. Tokens + chrome utilities live in `src/styles/tokens.css` / `globals.css`; see `docs/design-language.md`. |
...
| Verification proof | `pnpm verify` walks every step of every topic in a real browser, asserts narration + key stage state, and writes screenshot storyboards to `verification/`: contact sheets, `steps.json` and `index.html` are committed (per-step PNGs are regenerated locally and git-ignored). This is the durable proof of animation quality.                                                                                                                                                                                                                                                                                                                                                                                    |
