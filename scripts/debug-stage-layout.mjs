...
const info = await page.evaluate(() => {
  ...
  return {
    ...
    actors: { rect: r(actors), gtc: cs(actors, 'grid-template-columns'), gta: cs(actors, 'grid-template-areas'), flow: cs(actors, 'grid-auto-flow'), layout: actors.getAttribute('data-layout') },
    cards: [...document.querySelectorAll('[data-actor]')].map((el) => ({ id: el.getAttribute('data-actor'), slot: el.getAttribute('data-slot'), rect: r(el), area: cs(el, 'grid-area'), display: cs(el, 'display'), opacity: cs(el, 'opacity'), transform: cs(el, 'transform'), visibility: cs(el, 'visibility') })),
  }
})
