...
| Storyboards (durable proof) | `pnpm verify`                               | For every topic: a screenshot of every step (stage + narration + transport), `steps.json`, a contact sheet (`contact.png`), a dark-theme sheet for the first topic of each unit, and `verification/index.html` linking everything. Committed to the repo. |
